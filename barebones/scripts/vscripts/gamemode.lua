...
GAME_WAVE = 1
BOSS_COUNT = 1
LINE_COUNT = 1 -- номер текущего раунда
MAX_ROUNDS = 5 -- номер конечного раунда
ROUND_UNITS = 4 -- кол-во юнитов на 1 раунде
COUNT_SPAWN = 0
COUNT_ENEMY = 0
MAX_ENEMY = 30
PLAYERS = 0
...
function barebones:OnGameInProgress()
	...
	local return_time = 20 -- Записываем в переменную значение '20' это время между волнами
	local num_unit = 1
	local boss = CreateUnitByName("npc_neutral_boss_1",Entities:FindByName( nil, "boss_spawn"):GetAbsOrigin(), true, nil, nil, DOTA_TEAM_NEUTRALS )
	boss:SetInitialGoalEntity( Entities:FindByName( nil, "Boss_path") )
	Timers:CreateTimer(10, function() -- Создаем таймер, который запустится через 10 секунд после начала матча и запустит следующую функцию.
	if COUNT_ENEMY > MAX_ENEMY then
		GameRules:SetGameWinner(DOTA_TEAM_GOODGUYS)
		--Say(nil, "game end", true)
	end
		if COUNT_SPAWN == 32 then
		GAME_WAVE = GAME_WAVE + 1
		LINE_COUNT = LINE_COUNT + 1
		COUNT_SPAWN = 0
		num_unit = math.random(1,8)
		if num_unit%2 == 0 then num_unit = num_unit-1 end
		--Say(nil, "Wave " .. GAME_WAVE, true)
		return 30
	end
	if LINE_COUNT == MAX_ROUNDS then
			LINE_COUNT = 1
			--Say(nil, "BOSS WAVE!", true)
			local unit = CreateUnitByName("npc_line_boss_" .. BOSS_COUNT, point, true, nil, nil, DOTA_TEAM_NEUTRALS)
			unit:SetInitialGoalEntity( waypoint ) 
			--BOSS_COUNT = BOSS_COUNT + 1
			barebones:CreepLevelUp(unit, GAME_WAVE)
			GAME_WAVE = GAME_WAVE + 1
			return 60
	end
	for i=1, ROUND_UNITS do -- Произведет нижние действия столько раз, сколько указано в ROUND_UNITS. То есть в нашем случае создаст 5 юнита.
		local unit1 = CreateUnitByName( "npc_line_creep_"..num_unit, Entities:FindByName( nil, "spawnerino"..i):GetAbsOrigin() + RandomVector( RandomFloat( 0, 400 ) ), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit1:SetInitialGoalEntity( Entities:FindByName( nil, "way"..i) ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
		local unit2 = CreateUnitByName( "npc_line_creep_"..num_unit+1, Entities:FindByName( nil, "spawnerino"..i):GetAbsOrigin() + RandomVector( RandomFloat( 0, 400 ) ), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit2:SetInitialGoalEntity( Entities:FindByName( nil, "way"..i) ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
		COUNT_SPAWN = COUNT_SPAWN + 2
		COUNT_ENEMY = COUNT_ENEMY + 2
		if GAME_WAVE > 1 then
			barebones:CreepLevelUp(unit1, GAME_WAVE)
			barebones:CreepLevelUp(unit2, GAME_WAVE)
		end
	end
		return return_time -- Возвращаем таймеру время, через которое он должен снова сработать.
	end)
end
...
function barebones:OnEntityKilled(keys)
	...
	if killedUnit:IsRealHero() then
		...
		PLAYERS = PLAYERS + 1
	elseif unitname == "npc_neutral_boss_1" then
...
function barebones:CreepLevelUp(Creep, current_wave)
	local unitname = Creep:GetUnitName()
	local health = Creep:GetBaseMaxHealth()
	local damage = Creep:GetAttackDamage()
	local gold = Creep:GetGoldBounty()
	if unitname =="npc_line_creep_1" then
		health = health*current_wave+100
		damage = damage*current_wave-10*current_wave
	elseif unitname =="npc_line_creep_2" then
		health = health*current_wave+100
		damage = damage*current_wave-10*current_wave
	elseif unitname =="npc_line_creep_3" then
		health = health*current_wave
		damage = damage*current_wave-10*current_wave
	elseif unitname =="npc_line_creep_4" then
		health = health*current_wave
		damage = damage*current_wave-10*current_wave
	elseif unitname =="npc_line_creep_5" then
		health = health*current_wave
		damage = damage*current_wave-10*current_wave
	elseif unitname =="npc_line_creep_6" then
		health = health*current_wave
		damage = damage*current_wave-10*current_wave
	elseif unitname =="npc_line_creep_7" then
		health = health*current_wave
		damage = damage*current_wave-10*current_wave
	elseif unitname =="npc_line_creep_8" then
		health = health*current_wave
		damage = damage*current_wave-10*current_wave
	elseif unitname =="npc_line_boss_1" then
		health = health*current_wave
		damage = damage*current_wave-10*current_wave
	end
	  Creep:SetBaseMaxHealth(health)
		...
		Creep:SetBaseDamageMin(damage * 0.8)
		Creep:SetBaseDamageMax(damage * 1.2)
		Creep:SetMaximumGoldBounty(gold * current_wave)
		Creep:SetMinimumGoldBounty(gold * current_wave)
		Creep:SetDeathXP(100*current_wave)
end
